...
# Educational Module
    ...
    st.write("""
    **World Health Organization Guidelines for Adult Physical Activity (per week)**:
    - Engage in either:
        - Moderate exercise for 150-300 minutes, OR
        - Vigorous exercise for 75-150 minutes.
        - Ideally, the exercise should be spread throughout the week.
    
    **Benefits of Exercise**:
    - Improves cardiovascular health, strength, and flexibility
    - Boosts mood and reduces symptoms of depression and anxiety
    - Enhances cognitive function and reduces the risk of chronic diseases
    ...
    **Theories of Health Behavior Change**:
    - **Self-Efficacy**:
        ...
        - People are more likely to adopt behaviors if they:
            - Believe they can initiate and carry out the behavior (self-efficacy)
            - Believe that the behavior will produce valuable outcomes (outcome expectations)
        - Reciprocal determinism: Human functioning is influenced by the interaction of behavior, environment, and personal factors, particularly self-efficacy and cognitive processes.
        
        **Sources of Self-Efficacy**:
        - **Mastery Experiences (successes/failures)**:
            - Personal successes and failures shape an individual's confidence in their abilities.
            - Successes reinforce self-efficacy, while failures—if interpreted as learning experiences—can also build resilience and determination.
        
        - **Vicarious Experience (modeling)**:
            - Observing others (role models) successfully perform a behavior can enhance self-efficacy in those who feel similar to the model.
            - Seeing someone else succeed, especially someone they identify with, can increase the belief that they, too, can succeed.
        
        - **Social or Verbal Persuasion**:
            - Encouragement or feedback from others can build confidence.
            - Positive reinforcement from friends, coaches, or virtual sources strengthens self-efficacy by helping individuals believe they have the skills to succeed.
...
# Activity Log (Journaling)
    ...
    for day in days:
        activity_log[day] = st.text_area(f"{day} - Type of Activity & Minutes", key=f"log_{day}")
...
# Results Page
    ...
    if all(key in st.session_state for key in ['exercise_frequency', 'exercise_duration', 'cardio_minutes', 'weight_minutes']):
